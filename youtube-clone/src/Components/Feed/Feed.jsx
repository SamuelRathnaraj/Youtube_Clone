...
const Feed = ({category}) => {
  ...
  return (
    <div className="feed">
        {data.map((item,index)=>{
            return(
            <Link key={index}  to={`videos/${item.snippet.categoryId}/${item.id}`} className='card'>
             <img src={item.snippet.thumbnails.medium.url} alt="" />
             <h2>{item.snippet.title}</h2>
             <h3>{item.snippet.channelTitle}</h3>
             <p>{value_coverter(item.statistics.viewCount)} views &bull; {moment(item.snippet.publishedAt).fromNow()}</p>
             </Link>
            )
        })}
